...
const SearchBar = ({ onSearch }) => {
  ...
  const [commitsData, setCommitsData] = useState(() => {
    const savedData = localStorage.getItem("commitsData");
  ...
  const fetchCommitsData = async (username) => {
    try {
      ...
      localStorage.setItem("commitsData", JSON.stringify(commitsResults));
    } catch (error) {
      console.error("Error fetching commits:", error);
    } finally {
      setLoading(false);
  ...
  // Effect to refetch data on page refresh if username exists
  useEffect(() => {
    if (username) {
      fetchCommitsData(username);
    }
  }, [username]);
